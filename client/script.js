    ...
    axios.get(`http://localhost:3000/animals`)
        .then((res) => {
            res.data.forEach((ele) => {
                document.querySelector(`body`).appendChild(document.createElement(`p`)).textContent = ele
            })
        })
}
...
const repeatMyParam = () => {
    axios.get(`http://localhost:3000/repeat/'Test1'`)
        .then((response) => {
...
const queryRequest = () => {
    axios.get(`http://localhost:3000/query-test/?Whatever`)
    .then((res) => {
...
let foodContainer = document.querySelector(`ul`)
